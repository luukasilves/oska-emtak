...
|---|---|---|---|---|---|
| **Anthropic Economic Index (AEI)** (Handa et al., Feb 2025 + quarterly updates) | **Measured Claude usage** classified into augmentation vs automation per O*NET task; aggregates to SOC | O*NET task → SOC → ISCO via crosswalk | Yes — huggingface.co/datasets/Anthropic/EconomicIndex (CC-BY) | **Primary augmentation/automation split** | **Yes (primary, 1P-API release)** |
| **ILO WP140** (2025) | Expert-rated GenAI exposure gradient 1–4, ISCO-08 native | ISCO-08 4-digit (~427 occs) | Yes — ilo.org WP140 appendix | **Primary ISCO-native exposure baseline** *(target)* | No — next-step A |
| **Cazzaniga et al.** (IMF Staff Discussion Note 2024/001) | Updated Pizzinelli — Felten exposure × complementarity; 40 OECD countries | ISCO-08 | Yes — IMF PDF + occupation-level appendix | Cross-validation; complementarity-based axis split as backup | No — next-step A |
| **Felten et al. AIOE** (2021/2023) | Composite of 10 AI domains × O*NET abilities, expert-rated | SOC → crosswalk to ISCO-3 | Yes — github.com/AIOE-Data/AIOE | Most-cited baseline; use for sanity check / external comparability | **Yes (primary exposure baseline in v2)** |
...
| **JRC AIRE** (JRC145832, 2024) | EU-native AI exposure | ISCO-3 | Methodology yes; dataset access unclear | Validation only | No — next-step A |
...
**A — Register JRC Casas 2025/26 + ILO WP140 + Cazzaniga as additional exposure models.** Benchmark-grounded JRC Casas methodology (352 AI benchmarks → 14 cognitive abilities → 127 ISCO-3 occupations) is more current than Felten (anchored in pre-2021 AI patents). ILO WP140 is the human-expert rated ISCO-08 baseline the doc originally targeted. Cazzaniga adds complementarity cross-validation. All three compatible with our ISCO-2 aggregation. Adds expert-rated, measured-usage, ability-similarity, and benchmark-anchored sources side-by-side. Implementation: new aggregators in `src/fetch_ai_scores.py`, new rows in `MODELS.md`; pipeline auto-generates matrices and charts. Effort: ~0.5–1 day per source.
